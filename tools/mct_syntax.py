...
[실제로 틀렸던 것 — 기억으로 쓰지 말 것]
  ...
  CONLOAD   : 여섯 성분이 모두 0인 행이 하나만 있어도 파일 전체가 절점 0으로 임포트 실패 (conload()가 걸러냄)
  SECTION   : 한 블록 안에 여러 단면을 넣을 때 행 사이에 빈 줄을 두지 말 것
"""
...
def conload(case, loads):
    ...
    for (n, fx, fy, fz, mx, my, mz) in loads:
        if all(abs(v) < 1e-12 for v in (fx, fy, fz, mx, my, mz)):
            continue    # 실물 검증(2026-09-10): 여섯 성분이 모두 0인 절점하중 행이 하나라도 있으면 MCT 전체 임포트가 조용히 실패(절점 0)
