...
TEMP = 15.0                                   # 온도변화 ±15 ℃
APPUR = 0.49                                  # 부속설비 0.05 tonf/m² (2021 보고서) → kN/m²
SETTLE = 0.010                                # 지점침하 10 mm (확인 항목)
EDGE_A, EDGE_B = -1.95, 6.72                  # 연석 위치 (도로중심선 기준, G1측 / G2측)
Y_G = {"G1": 0.25, "G2": 4.52}                # 주형 위치 (도로중심선 기준)
T_EDGE, T_GIRDER, T_INNER = 0.300, 0.240, 0.240   # 바닥판 RC 두께: 캔틸레버 단부 0.30 → 주형 위 0.24, 내측 0.24(계산서 Tc). 헌치 60 mm는 별도 가산 (2021 보고서 W1~W16: 0.240~0.347)
...
def build():
    ...
    # 하중 (주형당) — 주형별 분담(Pt)과 편심 비틀림(Mt = −Σ w·e, e = y하중 − y주형, 우수법칙 +x)
    def deck_loads():
        """바닥판 RC·포장·부속설비를 횡방향으로 적분해 주형별 (w, mx) 산정. 단순보 분배: 각 주형이 연석~주형간 중앙까지 부담"""
        yG1, yG2 = Y_G["G1"], Y_G["G2"]; mid = (yG1 + yG2) / 2
        def t_rc(y):
            if y <= yG1: return T_EDGE + (T_GIRDER - T_EDGE) * (y - EDGE_A) / (yG1 - EDGE_A)
            if y >= yG2: return T_GIRDER + (T_EDGE - T_GIRDER) * (y - yG2) / (EDGE_B - yG2)
            return T_INNER
        out = {}
        for g, (a, b) in {"G1": (EDGE_A, mid), "G2": (mid, EDGE_B)}.items():
            n = 400; dy = (b - a) / n; w_rc = w_pv = w_ap = 0.0; m_rc = m_pv = m_ap = 0.0
            for i in range(n):
                y = a + (i + 0.5) * dy; e = y - Y_G[g]
                q_rc = t_rc(y) * G_RC * dy; q_pv = T_PAVE * G_PAVE * dy; q_ap = APPUR * dy
                w_rc += q_rc; w_pv += q_pv; w_ap += q_ap; m_rc -= q_rc * e; m_pv -= q_pv * e; m_ap -= q_ap * e
            w_rc += 2 * BF * TH * G_RC                       # 헌치 (주형 위, 편심 0)
            e_bar = (EDGE_A + 0.25 - Y_G[g]) if g == "G1" else (EDGE_B - 0.25 - Y_G[g])   # 방호벽 도심: 연석에서 0.25 m
            out[g] = dict(w_slab=w_rc, mx_slab=m_rc, w_sdl=w_pv + w_ap + BARRIER[g], mx_sdl=m_pv + m_ap - BARRIER[g] * e_bar)
        return out
    DL = deck_loads()
    w_slab = {g: DL[g]["w_slab"] for g in DL}; mx_slab = {g: DL[g]["mx_slab"] for g in DL}
    w_pave = T_PAVE * (B_DECK / 2) * G_PAVE
    sdl = {g: DL[g]["w_sdl"] for g in DL}; mx_sdl = {g: DL[g]["mx_sdl"] for g in DL}
    ...
    for tag, kind in (("A_steel", "steel"), ("C_comp", "comp")):
        ...
        if kind == "steel":
            ...
            loads += beamload_block("SLAB", [(e, -w_slab[g]) for e, m, sc, n1, n2, g, s1, s2 in elems])
            comb = "\n*LOADCOMB    ; Combinations\n" + loadcomb("D1", [("ST", "DEAD", 1), ("ST", "SLAB", 1)])
        else:
            cases = [("SDL", "D"), ("W", "W"), ("WL", "W"), ("CF", "CF"), ("LF", "L"), ("TP", "T"), ("TM", "T")]
            cases += [("UNITM", "D")] + [(f"SD_{p}", "D") for p in ("A1", "P1", "P2", "P3", "P4", "P5")]
            loads = "\n*STLDCASE    ; Static Load Cases\n" + "".join(f"   {nm:<10s}, {ty} , \n" for nm, ty in cases)
            ...
            loads += f"\n*USE-STLD, TM\n*SYSTEMPER    ; System Temperature\n   {-TEMP:.1f}\n"
            # 단위 단부모멘트 UNITM: A1·P5 주형 끝 절점 MY = ±1 kN·m (2차 부정정력 계수 k 산정용)
            loads += "\n*USE-STLD, UNITM\n*CONLOAD    ; Nodal Loads\n"
            for g in ("G1", "G2"):
                loads += f"   {nG[g][0][0]}, 0, 0, 0, 0, 1.0, 0, \n   {nG[g][-1][0]}, 0, 0, 0, 0, -1.0, 0, \n"
    ...
    info = dict(spans=spans, total=tot, sup_s=sup_s, stations=stations, n_nodes=len(nodes), n_main=n_main, n_xb=len(xb_elems),
                ...
                mx_slab=mx_slab, mx_sdl=mx_sdl, settle=SETTLE, appur=APPUR,
...
if __name__ == "__main__":
    ...
    print("바닥판(kN/m/주형):", {g: round(v, 2) for g, v in I['w_slab'].items()}, "| 비틀림(kN·m/m):", {g: round(v, 2) for g, v in I['mx_slab'].items()})
    print("SDL(kN/m/주형):", {g: round(v, 2) for g, v in I['sdl'].items()}, "| 비틀림(kN·m/m):", {g: round(v, 2) for g, v in I['mx_sdl'].items()})
    print(f"W {I['w_wind']:.2f} | WL {I['w_wl']:.2f} | CF {I['w_cf']:.3f} | LF {I['w_lf']:.3f} kN/m | T ±{I['temp']} ℃ | 침하 {I['settle']} m | 부속설비 {I['appur']} kN/m²")
